...
export class HashingUtil {
  ...
  async generatePassword(length = 12, customSpecialChars = ''): Promise<string> {
    try {
      ...
      const specialCharacters = customSpecialChars || '!@#$%^&*()_+-=[]{};\':"\\|,.<>/?';
      ...
      const categories: string[] = [lowercaseLetters, uppercaseLetters, numbers, specialCharacters.length > 0 ? specialCharacters : '']; // Include special characters only if they exist
      const chosenCategories = new Set<string>();
      while (chosenCategories.size < Math.min(4, categories.length)) { // Ensure at least 3 (or less if no special characters) categories
